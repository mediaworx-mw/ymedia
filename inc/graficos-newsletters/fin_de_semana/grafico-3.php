...
var max = Math.max(...datosGraficosX['Cuota de las tem. en abierto - Top5'].map( x => x["Cuota (%)"] ).filter( x => x !== undefined).map(x => typeof x === 'string' ? Number(x.replace(/,/g, '.').replace(/%/, '')) : x));
