...
function  graficoEGM9() {
  
  am4core.useTheme(am4themes_animated);

  // Create chart instance
  var chart = am4core.create("grafico-EGM-9", am4charts.XYChart);
  
  var sheet_name = 'RadioGenMañana';
  ...
  input = input.map(x => {
    const moreData = x["Radio generalista - Mañana"] !== undefined ? enMedios(x["Radio generalista - Mañana"], medios) : false;
    x[col1] = clean(x[col1]);
    x[col2] = clean(x[col2]);
    x[col3] = clean(x[col3]);
    x[col4] = clean(x[col4]);
  ...
  var category = "Radio generalista - Mañana";
...
var graficoEGM9_show = false;

jQuery('#grafico-EGM-9').waypoint(function() {
  if(!graficoEGM9_show) {
    graficoEGM9();
  }
  graficoEGM9_show = true;
}, {
  offset: '75%'
});
